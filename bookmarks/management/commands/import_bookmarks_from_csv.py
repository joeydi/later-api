...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("file_path", type=str, help="Path to the export file")
        parser.add_argument("user_id", type=str, help="User ID to assign the Bookmarks")
        ...
        with open(file_path) as file:
            ...
            for row in csv_reader:
                if Bookmark.objects.filter(url=row["URL"], user=user).exists():
                    self.stdout.write(
                        self.style.WARNING(
                            'Bookmark already exists: "%s"' % row["Title"]
                        )
                    )
                    continue
                ...
                bookmark = {
                    ...
                    "title": textwrap.shorten(
                        row["Title"], width=255, placeholder="..."
                    ),
                ...
                self.stdout.write(
                    self.style.SUCCESS('Importing bookmark: "%s"' % row["Title"])
                )
